...
DeadRecall 會新增自己的創造模式頁籤，集中顯示模組新增物品，包含銅板手、各級背包、死亡背包、硝石、豬糞、木灰、缽與帶硫磺的缽。
...
| 防火 | ✅ 背包本身防火，物品不會被燒毀 |
| 存活時間 | 永久（不會自然消失） |
| 虛空保護 | ✅ 低於虛空傷害線時會取消原版移除，獲得小幅向上動量與緩降，不會被虛空移除 |
| 定位光柱 | ✅ 地上的死亡背包會顯示類似烽火台的紅色光柱 |
| Tooltip | 顯示物品數量與排數 |
...
| 等級 | 物品 ID | 容量 | 本體防護 | 取得方式 | 模板 | 基底 | 附加材料 |
|------|---------|------|----------|---------|------|------|----------|
| 🟤 **基礎背包** | `backpack_basic` | 9格（1排） | 無額外防護；被破壞或時間到時內容物掉出（虛空除外） | 鍛造台升級 | 束口袋 | 束口袋 | 皮革 |
| 🟦 **標準背包** | `backpack_standard` | 18格（2排） | 抵抗仙人掌；其他破壞掉出內容物（虛空除外） | 鍛造台升級 | 束口袋 | 基礎背包 | 鐵錠 |
| ⚫ **進階背包** | `backpack_advanced` | 27格（3排） | 抵抗仙人掌與爆炸；其他破壞掉出內容物（虛空除外） | 鍛造台升級 | 束口袋 | 標準背包 | 鑽石 |
| 🟣 **獄髓背包** | `backpack_netherite` | 36格（4排） | 防火、熔岩、仙人掌、爆炸與自然消失；虛空不保護 | 鍛造台升級 | 獄髓升級模板 | 進階背包 | 獄髓錠 |

#### 背包特性
- ✅ 右鍵使用開啟背包界面
- ✅ 物品保存（關閉後不丟失）
- ✅ Tooltip 顯示等級、格數、使用量與本體防護
- ✅ 每次只能堆疊 1 個（不可疊放）
- ✅ Shift+點擊快速移動物品

#### 背包物品被破壞時
- 背包本體防護會隨等級成長：基礎不額外防護、標準抵抗仙人掌、進階抵抗仙人掌與爆炸、獄髓抵抗火焰/熔岩/仙人掌/爆炸/自然消失
- 死亡背包使用獨立最高防護：免疫傷害、自然消失與虛空；低於虛空傷害線時會取消原版移除，獲得小幅向上動量並限制下墜速度
- 一般等級背包的虛空不受防護，背包與內容物都不會被救回
- 背包物品實體若因未被防護的爆炸、仙人掌、火焰或自然時間到而被移除，背包內物品會掉出來
...
- 整理玩家背包時只處理主背包，不會移動快捷列、盔甲、盾牌/副手、身體裝備或鞍等裝備欄位
- 可到 **設定 → 按鍵綁定 → DeadRecall** 重新指定快捷鍵

#### 背包配方與進度
- 取得束口袋後，會解鎖所有 DeadRecall 背包鍛造升級配方

| 進度名稱 | 觸發條件 | 說明 |
|----------|----------|------|
| DeadRecall | 取得束口袋或豬糞 | 開啟 DeadRecall 進度頁 |
| 小包起步 | 取得基礎背包 | 製作第一級背包 |
| 再塞一點 | 取得標準背包 | 製作第二級背包 |
| 不是倉庫，是背包 | 取得進階背包 | 製作第三級背包 |
| 防火行李 | 取得獄髓背包 | 製作最高級防火背包 |
| 第一份工作合約 | 首次成功替銅魁儡綁定容器 | 啟動銅魁儡分類流程 |
| 禮輕味重 | 用豬糞砸中任一生物 | 投擲者完成 |
| 誰掉到茅坑裡啦 | 玩家被豬糞砸中 | 被砸到的玩家完成 |
| 有缽而來 | 合成缽 | 製作煉金器皿 |
...
| 豬糞 | `pig_manure` | 用鏟子左鍵帶豬糞的泥土系列方塊取得，也可右鍵投擲 |
| 木灰 | `wood_ash` | 乾草捆放入熔爐燒製取得 |
| 裝可可粉的缽 | `cocoa_powder` | 可可豆 + 糖 + 缽無序合成取得，倒入煉藥鍋後回傳空缽 |
| 熱可可 | `hot_cocoa` | 由牛奶、裝可可粉的缽與丟入的糖在營火上的煉藥鍋煮成，使用玻璃瓶裝取 |
| 櫻花釀 | `cherry_brew` | 由糖、櫻花樹葉、發光莓與甜莓在營火上的滿水煉藥鍋釀成 |
| 缽 | `stone_bowl` | 對硫磺方塊使用後可裝填硫磺 |
| 帶硫磺的缽 | `sulfur_bowl` | 作為煉金火藥材料，合成後回傳缽 |

> 取得燧石會解鎖缽的合成配方；合成缽會完成進度「有缽而來」。

#### 豬糞取得
- 豬會加入類似羊吃草的 AI，成年豬約每 1000 tick 嘗試一次，幼年豬約每 50 tick 嘗試一次
- 豬吃草觸發時，腳下的泥土系列方塊會變成「帶豬糞」版本
- 目前支援：泥土、草地、砂土、扎根土、灰壤、菌絲土、泥巴
- 手持任意鏟子左鍵帶豬糞方塊，會取得 1 個豬糞，方塊回復成乾淨狀態
- 手持豬糞右鍵會像雪球一樣丟出；命中草地方塊會把草地變成帶豬糞的草地
- 豬糞命中玩家或其他生物時會套用「臭味」狀態，期間移動速度降低，附近村民、貓狗與其他動物會遠離
- 用豬糞砸中任一生物會完成進度「禮輕味重」
- 玩家被豬糞砸中會完成進度「誰掉到茅坑裡啦」
...
硝石煉藥鍋配方資料：
```
src/main/resources/data/deadrecall/deadrecall/cauldron_recipes/saltpeter.json
```

#### 熱可可煮製
1. 使用可可豆 + 糖 + 缽無序合成裝可可粉的缽
2. 在煉藥鍋正下方放一個已點燃的營火或靈魂營火
3. 手持牛奶桶右鍵空煉藥鍋，牛奶倒入後回傳空桶
4. 手持裝可可粉的缽右鍵已倒入牛奶的煉藥鍋，倒入後回傳空缽
5. 將糖丟進鍋中；糖只接受丟入，不接受右鍵投入
6. 鍋子會在營火上加熱，但不會像硝石煉製一樣消耗層數或燒乾
7. 完成時會播放提示音；提示音響完後才可用玻璃瓶裝取熱可可
8. 每個玻璃瓶裝出 1 瓶熱可可並消耗 1 層鍋內份量，3 瓶裝完後回到空煉藥鍋

熱可可煉藥鍋配方資料：
```
src/main/resources/data/deadrecall/deadrecall/cauldron_recipes/hot_cocoa.json
```

#### 櫻花釀煮製
1. 在煉藥鍋正下方放一個已點燃的營火或靈魂營火
2. 把煉藥鍋裝滿水（3 層水）
3. 加入糖、櫻花樹葉、發光莓、甜莓各 1 份
4. 全部投入後會開始釀製；完成時播放提示音
5. 使用玻璃瓶裝取櫻花釀，每瓶消耗 1 層鍋內份量，3 瓶裝完後回到空煉藥鍋
6. 飲用櫻花釀會獲得「櫻花綻放」3 分鐘：移動速度 +5%，身邊會緩緩落下櫻花粒子
7. 帶有「櫻花綻放」效果時攻擊生物，目標身上也會產生櫻花粒子效果

櫻花釀煉藥鍋配方資料：
```
src/main/resources/data/deadrecall/deadrecall/cauldron_recipes/cherry_brew.json
```

#### 煉藥鍋配方 JSON 格式
煉藥鍋配方不是原版工作台 `recipe/`，而是 DeadRecall 自訂資料：
```
data/<namespace>/deadrecall/cauldron_recipes/<recipe_id>.json
```

主要欄位：
| 欄位 | 說明 |
|------|------|
| `start` | 起始鍋子，支援 `empty_cauldron`、`full_water_cauldron` |
| `initial_level` | 轉成煉製中煉藥鍋後的層數，通常為 3 |
| `requires_lit_campfire` | 是否需要正下方有點燃營火才可開始與推進烹煮 |
| `cook_mode` | `per_ingredient` 每份材料各煮一次；`after_all_inputs` 全部投入後煮一次 |
| `cook_ticks` | 每次烹煮所需 tick |
| `consume_level_per_cook` | 每次煮完是否扣 1 層，硝石為 true，熱可可為 false |
| `ingredients` | 材料清單，每筆可指定 `item` 或 `items`、投入後 `remainder`、訊息與音效 |
| `result` | 完成結果，支援 `drop_item` 與 `bottled_item` |

右鍵投入與丟物品進鍋現在共用同一套配方判定；材料可用 `allow_right_click`、`allow_dropped` 控制是否接受特定投入方式。材料也可用 `can_start` 控制是否允許該材料建立新的煉製中煉藥鍋，例如熱可可只能由牛奶桶開鍋。
...
- ✅ 取得束口袋會解鎖背包配方，並新增每級背包製作與豬糞命中生物的進度
...
- ✅ 修正銅魁儡扳手開啟綁定清單時，LLM 管理權限檢查在 Minecraft 26.2 可能因映射方法不相容導致伺服器崩潰的問題
...
│   │   ├── ModItemGroups.java         # DeadRecall 創造模式頁籤與物品清單
...
│   ├── mixin/
│   │   ├── EntityMixin.java           # 死亡背包低於虛空傷害線時取消原版移除並給予向上動量
...
│   ├── saltpeter.json                 # 營火煉藥鍋：木灰 + 蘑菇 + 豬糞 => 硝石
│   ├── hot_cocoa.json                 # 營火煉藥鍋：牛奶 + 裝可可粉的缽 + 丟入糖 => 熱可可
│   └── cherry_brew.json               # 營火煉藥鍋：糖 + 櫻花樹葉 + 發光莓 + 甜莓 => 櫻花釀
...
- 客戶端 `ItemEntityRendererMixin` 會在渲染死亡背包物品實體時提交一條原版 `BeaconRenderer` 光柱；此效果只影響顯示，不改背包資料或拾取邏輯
...
- `BackpackItemHelper` 統一判斷 `TieredBackpackItem` 與 `DeathBackpackItem`，負責背包防護等級、一般背包虛空排除、死亡背包虛空保護與把 `DataComponents.CONTAINER` 內物品生成回世界
- `ItemEntityMixin` 攔截背包物品實體被傷害破壞與自然時間到消失，先依背包等級阻擋可防護的破壞，未防護且非虛空時才於原本 `discard()` 前掉出內部物品
- `EntityMixin` 攔截原版 `checkBelowWorld()`，死亡背包低於虛空傷害線時會取消原版 `discard()` 並給予小幅向上動量；`ItemEntityMixin` 會限制死亡背包在世界底部以下的下墜速度，模擬緩降
...
- `PigManureBlock` 會記住原本的乾淨方塊狀態，鏟子左鍵採集後可還原原方塊
- `AlchemyCauldronRecipes` 透過資料包重載讀取 `data/<namespace>/deadrecall/cauldron_recipes/*.json`
- `AlchemyHandler.register()` 註冊 `AttackBlockCallback`、`UseBlockCallback` 與 server tick：
  - 鏟子左鍵帶豬糞方塊：還原方塊並給玩家 1 個 `pig_manure`
  - 右鍵煉藥鍋時，使用通用配方流程判斷可投入材料、容器回傳與瓶裝結果
  - 每 5 tick 掃描掉進煉藥鍋的物品實體，使用同一套通用配方流程處理丟入材料
- `pig_manure` 右鍵會生成帶豬糞物品外觀的投擲物；命中草地會生成帶豬糞草地，命中生物會套用「臭味」狀態
- `PigManureInteractions` 每 10 tick 掃描臭味目標，讓附近 8 格內的動物與村民往反方向移動
- `CherryBrewInteractions` 每 12 tick 掃描帶有「櫻花綻放」效果的生物並生成櫻花粒子；帶效果的生物攻擊命中時，也會在目標身上生成櫻花粒子
- 第一份材料投入合法起始鍋子時，方塊會替換成 `alchemy_cauldron`
- `AlchemyCauldronBlockEntity` 儲存目前 `recipe_id`、已投入材料 ID、已煮入材料 ID、是否可裝瓶，以及目前烹煮時間
- 只有煉藥鍋正下方是已點燃的營火或靈魂營火時才會推進烹煮
- 每 200 tick 完成一份材料，並消耗 1 層水；三份材料完成後生成 1 個硝石並把方塊恢復成空煉藥鍋
- 熱可可與櫻花釀完成時只播放一次提示音，不會因加熱消耗鍋內層數；裝瓶時才扣除層數
